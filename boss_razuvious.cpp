...
SD%Complete: 99
SDComment: missing core patch to mindcontroll/posses
...
#define SPELL_DISRUPTINGSHOUT       55543
#define H_SPELL_DISRUPTINGSHOUT		29107
...
struct MANGOS_DLL_DECL boss_razuviousAI : public ScriptedAI
{
	...
	void DamageTaken(Unit *done_by,uint32 &damage)
	{
		...
		{
			DoCast(m_creature,SPELL_HOPELESS);
		}
	}
    ...
    void UpdateAI(const uint32 diff)
    {
        ...
        {
			DoCast(m_creature,IsHeroicMode ? H_SPELL_DISRUPTINGSHOUT : SPELL_DISRUPTINGSHOUT);
...
bool GOHello_Orb_Of_Domination(Player* pPlayer, GameObject* pGo)
{
    ScriptedInstance* pInstance = (ScriptedInstance*)pGo->GetInstanceData();
	Creature* DeathKnight;
	DeathKnight = GetClosestCreatureWithEntry(pPlayer,NPC_DEATH_KNIGHT,100);
	pPlayer->SetCharm(DeathKnight);
	pPlayer->SetFarSightGUID(DeathKnight->GetGUID());
	pPlayer->SetClientControl(DeathKnight,1);
    return false;
}
...
void AddSC_boss_razuvious()
{
	...
	newscript = new Script;
    newscript->Name = "Orb_Of_Domination";
    newscript->pGOHello = &GOHello_Orb_Of_Domination;
    newscript->RegisterSelf();
}
